...
func buildForPrepBuiltinBody() []byte {
	assembler := amd64.NewAssembler(224)
	errorPath := assembler.NewLabel()
...
func buildArithmeticBuiltinBody() []byte {
...
func buildCompareBuiltinBody() []byte {
	assembler := amd64.NewAssembler(32)
	emitExitCurrentBuiltinToRuntime(assembler, false)
	return assembler.Buffer().Bytes()
}
...
func buildConcatBuiltinBody() []byte {
	...
	assembler.Jcc(amd64.CondNotEqual, boundaryPath)
	assembler.AddRegImm32(amd64.RegR8, 1)
	emitLoadBuiltinScratch64(assembler, amd64.RegR9, builtinScratchPayload1Offset)
	assembler.CmpRegReg(amd64.RegR8, amd64.RegR9)
	assembler.Jcc(amd64.CondBelowEqual, loop)
...
func buildLuaCallBuiltinBody() []byte {
	...
	callTargetReady := assembler.NewLabel()
	...
	resolvedFixedArgCopy := assembler.NewLabel()
	resolvedOpenArgCopy := assembler.NewLabel()
	copyCountReady := assembler.NewLabel()
	resolvedCopyCountReady := assembler.NewLabel()
	...
	uncoveredCall := assembler.NewLabel()
	callFeedbackMiss := assembler.NewLabel()
	directHostCandidate := assembler.NewLabel()
	directHostCall := assembler.NewLabel()
	tryResolvedHostCall := assembler.NewLabel()
	resolvedHostCall := assembler.NewLabel()
	megamorphicCallBoundary := assembler.NewLabel()
	...
	assembler.Jcc(amd64.CondEqual, directHostCandidate)
	emitMatchResolvedLuaClosureFeedback(assembler, feedback.SlotCall, tryResolvedHostCall)
	emitStoreBuiltinScratch64(assembler, builtinScratchPayload3Offset, amd64.RegRCX)
	assembler.Jmp(callTargetReady)

	_ = assembler.Bind(directHostCandidate)
	emitMatchDirectHostFunctionFeedback(assembler, feedback.SlotCall, callFeedbackMiss)
	assembler.Jmp(directHostCall)

	_ = assembler.Bind(directHostCall)
	emitReturnHostCallDispatch(assembler, amd64.RegRDX, hostCallProtocolDirect)

	_ = assembler.Bind(tryResolvedHostCall)
	emitMatchResolvedHostFunctionFeedback(assembler, feedback.SlotCall, callFeedbackMiss)
	assembler.Jmp(resolvedHostCall)

	_ = assembler.Bind(resolvedHostCall)
	emitReturnHostCallDispatch(assembler, amd64.RegRDX, hostCallProtocolResolvedPrependCallee)

	_ = assembler.Bind(callFeedbackMiss)
	emitBranchMegamorphicCallFeedback(assembler, feedback.SlotCall, megamorphicCallBoundary)

	_ = assembler.Bind(uncoveredCall)
	emitUpdateCallFeedbackIneligible(assembler, feedback.SlotCall)
	assembler.Jmp(runtimeDispatch)

	_ = assembler.Bind(megamorphicCallBoundary)
	emitReturnMegamorphicCallDispatch(assembler)
	...
	assembler.XorRegReg(amd64.RegRAX, amd64.RegRAX)
	emitStoreBuiltinScratch64(assembler, builtinScratchPayload3Offset, amd64.RegRAX)

	_ = assembler.Bind(callTargetReady)
	...
	assembler.CmpRegImm32(amd64.RegRAX, 0)
	assembler.Jcc(amd64.CondNotEqual, resolvedFixedArgCopy)
	...
	emitLoadBuiltinScratch64(assembler, amd64.RegRSI, builtinScratchPayload0Offset)
	assembler.AddRegImm32(amd64.RegRSI, 1)
	assembler.MoveRegReg(amd64.RegRBX, amd64.RegR9)
	emitCopyCallArguments(assembler, amd64.RegRBX, amd64.RegR12, amd64.RegRSI, amd64.RegRCX, amd64.RegR8)
	assembler.Jmp(callArgsCopied)

	_ = assembler.Bind(resolvedFixedArgCopy)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegR10)
	assembler.AddRegImm32(amd64.RegRCX, -1)
	assembler.CmpRegReg(amd64.RegRCX, amd64.RegRSI)
	assembler.Jcc(amd64.CondBelowEqual, resolvedCopyCountReady)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRSI)

	_ = assembler.Bind(resolvedCopyCountReady)
	assembler.MoveRegReg(amd64.RegRBP, amd64.RegRCX)
	assembler.AddRegImm32(amd64.RegRBP, 1)
	assembler.MoveRegReg(amd64.RegRBX, amd64.RegR9)
	emitLoadBuiltinScratch64(assembler, amd64.RegRAX, builtinScratchPayload3Offset)
	assembler.MoveMemReg64(amd64.RegRBX, 0, amd64.RegRAX)
	assembler.AddRegImm32(amd64.RegRBX, int32(value.TValueSize))
	emitLoadBuiltinScratch64(assembler, amd64.RegRSI, builtinScratchPayload0Offset)
	assembler.AddRegImm32(amd64.RegRSI, 1)
	emitCopyCallArguments(assembler, amd64.RegRBX, amd64.RegR12, amd64.RegRSI, amd64.RegRCX, amd64.RegR8)
	assembler.Jmp(callArgsCopied)
	...
	emitLoadBuiltinScratch64(assembler, amd64.RegRAX, builtinScratchPayload3Offset)
	assembler.CmpRegImm32(amd64.RegRAX, 0)
	assembler.Jcc(amd64.CondNotEqual, resolvedOpenArgCopy)
	assembler.MoveRegMem32(amd64.RegRCX, amd64.RegR13, rtstate.CallFrameTopOffset)
	assembler.AndRegImm32(amd64.RegRCX, 0xFFFF)
	emitLoadBuiltinScratch64(assembler, amd64.RegRSI, builtinScratchPayload0Offset)
	assembler.AddRegImm32(amd64.RegRSI, 1)
	assembler.XorRegReg(amd64.RegRBP, amd64.RegRBP)
	emitCountOpenCallArguments(assembler, amd64.RegRSI, amd64.RegRCX, amd64.RegRBP, amd64.RegR10)
	emitLoadBuiltinScratch64(assembler, amd64.RegRSI, builtinScratchPayload0Offset)
	assembler.AddRegImm32(amd64.RegRSI, 1)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRBP)
	assembler.MoveRegReg(amd64.RegRBX, amd64.RegR9)
	...
	_ = assembler.Bind(resolvedOpenArgCopy)
	assembler.MoveRegMem32(amd64.RegRCX, amd64.RegR13, rtstate.CallFrameTopOffset)
	assembler.AndRegImm32(amd64.RegRCX, 0xFFFF)
	emitLoadBuiltinScratch64(assembler, amd64.RegRSI, builtinScratchPayload0Offset)
	assembler.AddRegImm32(amd64.RegRSI, 1)
	assembler.XorRegReg(amd64.RegRBP, amd64.RegRBP)
	assembler.MoveRegReg(amd64.RegR8, amd64.RegR10)
	assembler.AddRegImm32(amd64.RegR8, -1)
	emitCountOpenCallArguments(assembler, amd64.RegRSI, amd64.RegRCX, amd64.RegRBP, amd64.RegR8)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRBP)
	assembler.AddRegImm32(amd64.RegRBP, 1)
	assembler.MoveRegReg(amd64.RegRBX, amd64.RegR9)
	emitLoadBuiltinScratch64(assembler, amd64.RegRAX, builtinScratchPayload3Offset)
...
func buildTailCallBuiltinBody() []byte {
	...
	callDeopt := assembler.NewLabel()
	callError := assembler.NewLabel()
	uncoveredTailCall := assembler.NewLabel()
	callFeedbackMiss := assembler.NewLabel()
	directHostCandidate := assembler.NewLabel()
	directHostCall := assembler.NewLabel()
	tryResolvedHostCall := assembler.NewLabel()
	...
	assembler.CmpRegImm32(amd64.RegRAX, shiftedBoxedTag(value.TagHostFunctionRef))
	assembler.Jcc(amd64.CondEqual, directHostCandidate)
	emitMatchResolvedLuaClosureFeedback(assembler, feedback.SlotTailCall, tryResolvedHostCall)
	emitStoreBuiltinScratch64(assembler, builtinScratchPayload3Offset, amd64.RegRCX)
	assembler.Jmp(callTargetReady)

	_ = assembler.Bind(directHostCandidate)
	emitMatchDirectHostFunctionFeedback(assembler, feedback.SlotTailCall, callFeedbackMiss)
	assembler.Jmp(directHostCall)
	...
	_ = assembler.Bind(tryResolvedHostCall)
	emitMatchResolvedHostFunctionFeedback(assembler, feedback.SlotTailCall, callFeedbackMiss)
	assembler.Jmp(resolvedHostCall)
	...
	_ = assembler.Bind(callFeedbackMiss)
	emitBranchMegamorphicCallFeedback(assembler, feedback.SlotTailCall, megamorphicCallBoundary)

	_ = assembler.Bind(uncoveredTailCall)
...
func emitMatchResolvedLuaClosureFeedback(assembler *amd64.Assembler, slotKind feedback.SlotKind, missLabel *amd64.Label) {
	monomorphic := assembler.NewLabel()
	polymorphic := assembler.NewLabel()
	megamorphic := assembler.NewLabel()
	megamorphicSidecar := assembler.NewLabel()
	megamorphicInline := assembler.NewLabel()
	nextEntry := assembler.NewLabel()
	nextMegaEntry1 := assembler.NewLabel()
	nextMegaEntry2 := assembler.NewLabel()
	nextMegaEntry3 := assembler.NewLabel()
	done := assembler.NewLabel()

	emitLoadFeedbackCellBaseFromCallArg(assembler, amd64.RegR9, amd64.RegRAX, rtstate.StubCallBlockArg3Offset, missLabel)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellStateOffset)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StateMonomorphic, feedback.AccessCallResolvedLuaClosure, slotKind))
	assembler.Jcc(amd64.CondEqual, monomorphic)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StatePolymorphic, feedback.AccessInvalid, slotKind))
	assembler.Jcc(amd64.CondEqual, polymorphic)
	assembler.AndRegImm32(amd64.RegRAX, 0x00FF00FF)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StateMegamorphic, feedback.AccessInvalid, slotKind)&0x00FF00FF)
	assembler.Jcc(amd64.CondEqual, megamorphic)
	assembler.Jmp(missLabel)

	_ = assembler.Bind(monomorphic)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CellPayload32AOffset, feedback.CellPayload32BOffset, feedback.CellPayload32COffset, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)

	_ = assembler.Bind(megamorphic)
	emitBranchMegamorphicCallSidecar(assembler, amd64.RegR9, megamorphicSidecar, megamorphicInline)

	_ = assembler.Bind(megamorphicInline)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellAccessKindOffset)
	assembler.AndRegImm32(amd64.RegRAX, 0xFF)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CellPayload32AOffset, feedback.CellPayload32BOffset, feedback.CellPayload32COffset, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)

	_ = assembler.Bind(megamorphicSidecar)
	emitLoadCallPolymorphicDataBaseFromCell(assembler, amd64.RegR9, amd64.RegR9, missLabel)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CallPolyEntryAccessKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry1)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry1)
	assembler.Jmp(done)

	_ = assembler.Bind(nextMegaEntry1)
	assembler.AddRegImm32(amd64.RegR9, feedback.CallPolymorphicEntrySize)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CallPolyEntryAccessKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry2)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry2)
	assembler.Jmp(done)

	_ = assembler.Bind(nextMegaEntry2)
	assembler.AddRegImm32(amd64.RegR9, feedback.CallPolymorphicEntrySize)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CallPolyEntryAccessKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry3)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry3)
	assembler.Jmp(done)

	_ = assembler.Bind(nextMegaEntry3)
	assembler.AddRegImm32(amd64.RegR9, feedback.CallPolymorphicEntrySize)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CallPolyEntryAccessKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, missLabel)

	_ = assembler.Bind(polymorphic)
	emitLoadCallPolymorphicDataBaseFromCell(assembler, amd64.RegR9, amd64.RegR9, missLabel)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CallPolyEntryAccessKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedLuaClosure))
	assembler.Jcc(amd64.CondNotEqual, nextEntry)
	emitMatchResolvedLuaClosureEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextEntry)
	assembler.Jmp(done)
...
func emitBranchMegamorphicCallFeedback(assembler *amd64.Assembler, slotKind feedback.SlotKind, hitLabel *amd64.Label) {
	miss := assembler.NewLabel()

	emitLoadFeedbackCellBaseFromCallArg(assembler, amd64.RegR9, amd64.RegRAX, rtstate.StubCallBlockArg3Offset, miss)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellStateOffset)
	assembler.AndRegImm32(amd64.RegRAX, 0x00FF00FF)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StateMegamorphic, feedback.AccessInvalid, slotKind)&0x00FF00FF)
	assembler.Jcc(amd64.CondEqual, hitLabel)

	_ = assembler.Bind(miss)
}

func emitMatchDirectHostFunctionFeedback(assembler *amd64.Assembler, slotKind feedback.SlotKind, missLabel *amd64.Label) {
	monomorphic := assembler.NewLabel()
	polymorphic := assembler.NewLabel()
	megamorphic := assembler.NewLabel()
	megamorphicSidecar := assembler.NewLabel()
	...
	_ = assembler.Bind(monomorphic)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)
	...
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellAccessKindOffset)
	assembler.AndRegImm32(amd64.RegRAX, 0xFF)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry1)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry1)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry2)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry2)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry3)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry3)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, missLabel)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextEntry)
	emitMatchDirectHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextEntry)
	assembler.Jmp(done)
...
func emitMatchDirectHostFunctionEntry(assembler *amd64.Assembler, entryBaseReg amd64.Register, targetRefOffset int32, valueBitsOffset int32, mismatchLabel *amd64.Label) {
	assembler.MoveRegMem64(amd64.RegRAX, entryBaseReg, valueBitsOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegRDX)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	emitExtractHeapRefPayloadFromTValue(assembler, amd64.RegRCX, amd64.RegRDX)
	assembler.MoveRegMem64(amd64.RegRAX, entryBaseReg, targetRefOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegRCX)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
}

func emitMatchResolvedHostFunctionFeedback(assembler *amd64.Assembler, slotKind feedback.SlotKind, missLabel *amd64.Label) {
	monomorphic := assembler.NewLabel()
	polymorphic := assembler.NewLabel()
	megamorphic := assembler.NewLabel()
	megamorphicSidecar := assembler.NewLabel()
	...
	emitLoadFeedbackCellBaseFromCallArg(assembler, amd64.RegR9, amd64.RegRAX, rtstate.StubCallBlockArg3Offset, missLabel)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellStateOffset)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StateMonomorphic, feedback.AccessCallResolvedHostFunction, slotKind))
	assembler.Jcc(amd64.CondEqual, monomorphic)
	assembler.CmpRegImm32(amd64.RegRAX, feedback.PackCellPrefix(feedback.StatePolymorphic, feedback.AccessInvalid, slotKind))
	assembler.Jcc(amd64.CondEqual, polymorphic)
	assembler.AndRegImm32(amd64.RegRAX, 0x00FF00FF)
	...
	_ = assembler.Bind(monomorphic)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CellPayload32AOffset, feedback.CellPayload32BOffset, feedback.CellPayload32COffset, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)
	...
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR9, feedback.CellAccessKindOffset)
	assembler.AndRegImm32(amd64.RegRAX, 0xFF)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CellPayload32AOffset, feedback.CellPayload32BOffset, feedback.CellPayload32COffset, feedback.CellHeapRefOffset, feedback.CellValueBitsOffset, missLabel)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry1)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry1)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry2)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry2)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextMegaEntry3)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextMegaEntry3)
	assembler.Jmp(done)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, missLabel)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, missLabel)
	...
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.AccessCallResolvedHostFunction))
	assembler.Jcc(amd64.CondNotEqual, nextEntry)
	emitMatchResolvedHostFunctionEntry(assembler, amd64.RegR9, feedback.CallPolyEntryShapeVersionAOffset, feedback.CallPolyEntryShapeVersionBOffset, feedback.CallPolyEntryShapeKindOffset, feedback.CallPolyEntryTargetRefOffset, feedback.CallPolyEntryValueBitsOffset, nextEntry)
	assembler.Jmp(done)
...
func emitMatchResolvedLuaClosureEntry(assembler *amd64.Assembler, entryBaseReg amd64.Register, shapeVersionAOffset int32, shapeVersionBOffset int32, shapeKindOffset int32, targetRefOffset int32, valueBitsOffset int32, mismatchLabel *amd64.Label) {
	tableShape := assembler.NewLabel()
	hostObjectShape := assembler.NewLabel()
	typeMetatableShape := assembler.NewLabel()
	shapeMatched := assembler.NewLabel()

	assembler.MoveRegMem64(amd64.RegRAX, entryBaseReg, valueBitsOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegRDX)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	assembler.MoveRegMem32(amd64.RegRAX, entryBaseReg, shapeKindOffset)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.CallShapeTableMetatable))
	assembler.Jcc(amd64.CondEqual, tableShape)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.CallShapeHostObjectMetatable))
	assembler.Jcc(amd64.CondEqual, hostObjectShape)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.CallShapeTypeMetatable))
	assembler.Jcc(amd64.CondEqual, typeMetatableShape)
	assembler.Jmp(mismatchLabel)

	_ = assembler.Bind(tableShape)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRDX)
	emitExtractHeapRefPayloadFromTValue(assembler, amd64.RegR10, amd64.RegRCX)
	emitDecodeHeapRefFromRaw(assembler, amd64.RegR10)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR10, rttable.TableVersionOffset)
	assembler.MoveRegMem32(amd64.RegR8, entryBaseReg, shapeVersionAOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegR8)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	assembler.MoveRegMem64(amd64.RegR8, amd64.RegR10, rttable.MetatableOffset)
	assembler.MoveRegReg(amd64.RegRAX, amd64.RegR8)
	assembler.ShiftRightRegImm8(amd64.RegRAX, value.TagShift)
	assembler.CmpRegImm32(amd64.RegRAX, shiftedBoxedTag(value.TagTableRef))
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	emitExtractHeapRefPayloadFromTValue(assembler, amd64.RegR8, amd64.RegR8)
	emitDecodeHeapRefFromRaw(assembler, amd64.RegR8)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR8, rttable.TableVersionOffset)
	assembler.MoveRegMem32(amd64.RegR10, entryBaseReg, shapeVersionBOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegR10)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	assembler.Jmp(shapeMatched)

	_ = assembler.Bind(hostObjectShape)
	assembler.MoveRegReg(amd64.RegRAX, amd64.RegRDX)
	assembler.ShiftRightRegImm8(amd64.RegRAX, value.TagShift)
	...
	emitExtractHeapRefPayloadFromTValue(assembler, amd64.RegR10, amd64.RegRCX)
	emitDecodeHeapRefFromRaw(assembler, amd64.RegR10)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR10, rthost.WrapperMetatableVersionOffset)
	assembler.MoveRegMem32(amd64.RegR8, entryBaseReg, shapeVersionAOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegR8)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	assembler.MoveRegMem64(amd64.RegR8, amd64.RegR10, rthost.WrapperMetatableOffset)
	assembler.MoveRegReg(amd64.RegRAX, amd64.RegR8)
	assembler.ShiftRightRegImm8(amd64.RegRAX, value.TagShift)
	assembler.CmpRegImm32(amd64.RegRAX, shiftedBoxedTag(value.TagTableRef))
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	...
	_ = assembler.Bind(typeMetatableShape)
	emitLoadTypeMetatableKind(assembler, amd64.RegRDX, amd64.RegRAX, amd64.RegR10, mismatchLabel)
	assembler.MoveRegMem64(amd64.RegR8, amd64.VMStateRegister, rtstate.VMStateTypeMetatableStateOff)
	assembler.CmpRegImm32(amd64.RegR8, 0)
	assembler.Jcc(amd64.CondEqual, mismatchLabel)
	assembler.ShiftLeftRegImm8(amd64.RegRAX, 4)
	assembler.AddRegReg(amd64.RegR8, amd64.RegRAX)
	assembler.MoveRegMem32(amd64.RegRAX, amd64.RegR8, rtmeta.RegistryEntryVersionOffset)
	assembler.MoveRegMem32(amd64.RegR10, entryBaseReg, shapeVersionAOffset)
	assembler.CmpRegReg(amd64.RegRAX, amd64.RegR10)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	assembler.MoveRegMem64(amd64.RegRAX, amd64.RegR8, rtmeta.RegistryEntryMetatableOffset)
	assembler.MoveRegReg(amd64.RegR10, amd64.RegRAX)
	assembler.ShiftRightRegImm8(amd64.RegR10, value.TagShift)
	assembler.CmpRegImm32(amd64.RegR10, shiftedBoxedTag(value.TagTableRef))
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)
	emitExtractHeapRefPayloadFromTValue(assembler, amd64.RegRAX, amd64.RegRAX)
	emitDecodeHeapRefFromRaw(assembler, amd64.RegRAX)
	assembler.MoveRegMem32(amd64.RegR10, amd64.RegRAX, rttable.TableVersionOffset)
	assembler.MoveRegMem32(amd64.RegRAX, entryBaseReg, shapeVersionBOffset)
	assembler.CmpRegReg(amd64.RegR10, amd64.RegRAX)
	assembler.Jcc(amd64.CondNotEqual, mismatchLabel)

	_ = assembler.Bind(shapeMatched)
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRDX)
	assembler.MoveRegMem64(amd64.RegRDX, entryBaseReg, targetRefOffset)
	assembler.MoveRegImm64(amd64.RegRAX, uint64(value.BoxedMarker|(value.Raw(value.TagLuaClosureRef)<<value.TagShift)))
	assembler.AddRegReg(amd64.RegRDX, amd64.RegRAX)
}

func emitMatchResolvedHostFunctionEntry(assembler *amd64.Assembler, entryBaseReg amd64.Register, shapeVersionAOffset int32, shapeVersionBOffset int32, shapeKindOffset int32, targetRefOffset int32, valueBitsOffset int32, mismatchLabel *amd64.Label) {
	tableShape := assembler.NewLabel()
	hostObjectShape := assembler.NewLabel()
	typeMetatableShape := assembler.NewLabel()
	shapeMatched := assembler.NewLabel()
	...
	assembler.MoveRegReg(amd64.RegRCX, amd64.RegRDX)
	assembler.MoveRegMem64(amd64.RegRDX, entryBaseReg, targetRefOffset)
	assembler.MoveRegImm64(amd64.RegRAX, uint64(value.BoxedMarker|(value.Raw(value.TagHostFunctionRef)<<value.TagShift)))
	assembler.AddRegReg(amd64.RegRDX, amd64.RegRAX)
}

func emitLoadTypeMetatableKind(assembler *amd64.Assembler, valueReg amd64.Register, kindReg amd64.Register, scratchReg amd64.Register, missLabel *amd64.Label) {
	numberKind := assembler.NewLabel()
	nilKind := assembler.NewLabel()
	boolKind := assembler.NewLabel()
	stringKind := assembler.NewLabel()
	functionKind := assembler.NewLabel()
	threadKind := assembler.NewLabel()
	userDataKind := assembler.NewLabel()
	lightUserDataKind := assembler.NewLabel()
	kindReady := assembler.NewLabel()

	assembler.MoveRegReg(kindReg, valueReg)
	assembler.ShiftRightRegImm8(kindReg, 48)
	assembler.CmpRegImm32(kindReg, 0xFFFF)
	assembler.Jcc(amd64.CondNotEqual, numberKind)

	assembler.MoveRegReg(scratchReg, valueReg)
	assembler.ShiftRightRegImm8(scratchReg, value.TagShift)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagNil))
	assembler.Jcc(amd64.CondEqual, nilKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagBool))
	assembler.Jcc(amd64.CondEqual, boolKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagStringRef))
	assembler.Jcc(amd64.CondEqual, stringKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagLuaClosureRef))
	assembler.Jcc(amd64.CondEqual, functionKind)
	assembler.CmpRegImm32(scratchReg, uint32((uint64(value.BoxedMarker)>>value.TagShift)|uint64(value.TagHostFunctionRef)))
	assembler.Jcc(amd64.CondEqual, functionKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagNativeClosureRef))
	assembler.Jcc(amd64.CondEqual, functionKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagThreadRef))
	assembler.Jcc(amd64.CondEqual, threadKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagHostObjectRef))
	assembler.Jcc(amd64.CondEqual, userDataKind)
	assembler.CmpRegImm32(scratchReg, shiftedBoxedTag(value.TagLightHandle))
	assembler.Jcc(amd64.CondEqual, lightUserDataKind)
	assembler.Jmp(missLabel)

	_ = assembler.Bind(numberKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindNumber))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(nilKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindNil))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(boolKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindBoolean))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(stringKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindString))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(functionKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindFunction))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(threadKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindThread))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(userDataKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindUserData))
	assembler.Jmp(kindReady)

	_ = assembler.Bind(lightUserDataKind)
	assembler.MoveRegImm32(kindReg, uint32(rtmeta.KindLightUserData))

	_ = assembler.Bind(kindReady)
}

func emitBranchMegamorphicCallSidecar(assembler *amd64.Assembler, cellBaseReg amd64.Register, sidecarLabel *amd64.Label, inlineLabel *amd64.Label) {
	assembler.MoveRegMem32(amd64.RegRAX, cellBaseReg, feedback.CellStateOffset)
	assembler.AndRegImm32(amd64.RegRAX, uint32(feedback.CellFlagCallMegamorphicSidecar)<<24)
	assembler.CmpRegImm32(amd64.RegRAX, uint32(feedback.CellFlagCallMegamorphicSidecar)<<24)
	assembler.Jcc(amd64.CondEqual, sidecarLabel)
	assembler.Jmp(inlineLabel)
}
...
func emitLoadCallPolymorphicDataBaseFromCell(assembler *amd64.Assembler, cellBaseReg amd64.Register, dstReg amd64.Register, missLabel *amd64.Label) {
	assembler.MoveRegMem64(dstReg, cellBaseReg, feedback.CellHeapRefOffset)
	assembler.CmpRegImm32(dstReg, 0)
	assembler.Jcc(amd64.CondEqual, missLabel)
...
func emitReturnHostCallDispatch(assembler *amd64.Assembler, calleeReg amd64.Register, protocol uint32) {
	if calleeReg != amd64.RegRAX {
		assembler.MoveRegReg(amd64.RegRAX, calleeReg)
	}
	assembler.MoveMemReg32(amd64.RegR11, execCtxReserved0Off, amd64.RegRAX)
	assembler.ShiftRightRegImm8(amd64.RegRAX, 32)
	assembler.MoveMemReg32(amd64.RegR11, execCtxReserved1Off, amd64.RegRAX)
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved2Off, protocol)
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved3Off, 0)
	assembler.MoveMemImm32(amd64.RegR11, execCtxFlagsOffset, execCtxFlagResolvedHostCallBoundary)
	emitExitCurrentBuiltinToRuntime(assembler, false)
}

func emitReturnMegamorphicCallDispatch(assembler *amd64.Assembler) {
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved0Off, 0)
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved1Off, 0)
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved2Off, 0)
	assembler.MoveMemImm32(amd64.RegR11, execCtxReserved3Off, 0)
	assembler.MoveMemImm32(amd64.RegR11, execCtxFlagsOffset, execCtxFlagMegamorphicCallBoundary)
	emitExitCurrentBuiltinToRuntime(assembler, false)
}
